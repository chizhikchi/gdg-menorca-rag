"""Modified utils.py for local development

Add this modification to your existing utils.py validate_key function
"""
import base64
import io
import mimetypes
...
def validate_key(request):
    ...
    if os.getenv('ENVIRONMENT') == 'development' or not os.getenv('CLOUD_RUN_SERVICE'):
        return None  # Allow local access without key
    
    secret_key = "5nsor2t0jse8k85m"
    ...
    if error_title is not None:
        # Define the error message here since it might not be in your utils
        ker_error_msg = """Please open the app from
<a target='_blank'
  href='https://console.cloud.google.com/vertex-ai/studio/saved-prompts/locations/global/3063047530204889088?project=model-fastness-466612-t7&deploy=true'>
  Vertex AI Studio
